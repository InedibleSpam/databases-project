...
// Helper Function: Calculates remaining seats by checking sold tickets against plane capacity
async function calculateRemainingSeatsFromFlight(flight, seat_class) {
    ...
    const ticketsSold = await funcs.get_tickets_sold(flight.log_id, requestedClass);

    return Math.max(0, totalSeats - ticketsSold);
}
...
router.get("/api/search_flights_sorted", async (req, res) => {
    try {
        ...
        const flights_with_details_promises = raw_flights.map(async (flight) => {
            const total_price = flight.price * parseInt(num_tickets);
            
            const remaining_seats = await calculateRemainingSeatsFromFlight(
                flight,
                seat_class
            );
        ...
        const flights_with_details = await Promise.all(flights_with_details_promises);
